...
export const getAgency = (start, limit, search) => (dispatch) => {
  apiInstanceFetch
    .get(
      `agency/index?start=${start}&limit=${limit}&search=${
        search ? search : "ALL"
      }`
    )
    .then((res) => {
...
export const getAgencyHistory =
  (startDate, endDate, start, limit) => (dispatch) => {
    apiInstanceFetch
      .get(
        `history/allAgencyHistory?agencyId=&startDate=${startDate}&endDate=${endDate}&start=${start}&limit=${limit}`
      )
      .then((res) => {
        dispatch({
          type: GET_AGENCY_HISTORY,
          payload: res.agencyResults,
          total: res.total,
        });
      })
      .catch((error) => console.log(error));
...
//get salary sheet
export const getSalarySheet = (agencyCode) => (dispatch) => {
  axios
    .get(`/salarySheet/${agencyCode}`, {
