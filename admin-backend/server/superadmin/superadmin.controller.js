...
exports.signUp = async (req, res) => {
  try {
    ...
    if (data) {
      ...
      return res.status(200).json({ status: false, message: "Purchase code is invalid!!" });
    }
  } catch (error) {
    console.log(error);
...
exports.update = async (req, res) => {
  try {
    ...
    if (!admin) return res.status(200).json({ status: false, message: "Super Admin doesn't Exist!" });
...
exports.updatePassword = async (req, res) => {
  try {
    ...
    const updateResult = await SuperAdmin.updateOne({ _id: req.admin._id }, { $set: { password: hashedPassword } });
...
exports.setPassword = async (req, res, next) => {
  try {
    ...
    const admin = await SuperAdmin.findById(req.params.adminId);
    if (!admin) {
      return res.status(404).json({
        status: false,
        message: "Super Admin not found",
    ...
    const updated = await SuperAdmin.updateOne({ _id: req.params.adminId }, { $set: { password: hashedPassword } });
