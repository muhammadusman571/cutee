...
exports.index = async (req, res) => {
  try {
    const sticker = await Sticker.find().sort({ createdAt: -1 }).lean();

    if (!sticker) return res.status(200).json({ status: false, message: "No data found!" });
...
exports.store = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "Success!", sticker: stickers });
  } catch (error) {
    console.log(error);
...
exports.update = async (req, res) => {
  try {
    ...
    if (!sticker) {
      deleteFile(req.file);
      return res.status(200).json({ status: false, message: "Sticker does not Exist!" });
    }
...
exports.destroy = async (req, res) => {
  try {
    ...
    if (!sticker) return res.status(200).json({ status: false, message: "Sticker does not Exist!" });
