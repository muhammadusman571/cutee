const EntryBanner = require("./entryBanner.model");
...
exports.createBanner = async (req, res) => {
  try {
    ...
    let data = await new EntryBanner(dataObj).save();
...
exports.index = async (req, res) => {
  try {
    let data = await EntryBanner.find({ isDelete: false }).sort({
      createdAt: -1,
    });
...
exports.update = async (req, res) => {
  try {
    const model = await EntryBanner.findById(req.params.Id);
