...
exports.index = async (req, res) => {
  try {
    ...
    if (!level)
      return res
        .status(200)
        .json({ status: false, message: "Level data not found!" });
...
exports.updateAccessibleFunction = async (req, res) => {
  try {
    if (!req.body.levelId || !req.body.fieldName)
      return res
        .status(200)
        .json({ status: false, message: "Invalid Details!" });
    ...
    if (fieldName === "liveStreaming") {
      level.accessibleFunction.liveStreaming =
        !level.accessibleFunction.liveStreaming;
      ...
      level.accessibleFunction.uploadVideo =
        !level.accessibleFunction.uploadVideo;
    } else {
      level.accessibleFunction.uploadPost =
        !level.accessibleFunction.uploadPost;
    }
