...
exports.dashboard = async (req, res) => {
  try {
    const [totalUser, activeUser, vipUser, liveUser, post, video, report, coinRevenue, vipRevenue] = await Promise.all([
      User.find().countDocuments(),
      User.find({ isOnline: true }).countDocuments(),
      User.find({ isVIP: true }).countDocuments(),
      LiveUser.find().countDocuments(),
      Post.find().countDocuments(),
      Video.find().countDocuments(),
            ...
            pipeline: [
              {
                $match: {
                  $expr: {
                    $and: [{ $eq: ["$type", 2] }, { $eq: ["$planId", "$$planIds"] }],
                  },
                },
              },
            ],
