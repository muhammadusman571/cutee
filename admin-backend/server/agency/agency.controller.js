  ...
  try {
    ...
    if (agencyCode) {
      ...
      return res.status(200).json({
        status: false,
        message:
          "agency already exists with that agency code.Try with new agency code.",
      });
    }
    ...
    agency.ref = req.body.ref ? req.body.ref : null;
    await agency.save();
...
exports.update = async (req, res) => {
  try {
    ...
    agency.bankDetails = req.body.bankDetails
...
exports.activeOrNot = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: true, message: "Success", data: updatedAgency });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
exports.index = async (req, res) => {
  try {
    const user = req.admin; // assuming middleware sets req.user with role & _id
    ...
    // Search by agency name if provided
    if (search !== "ALL") {
...
exports.getAgency = async (req, res) => {
  try {
    const agency = await Agency.find({ isActive: true }).select(
      "_id name agencyCode"
    );
  ...
  try {
    ...
    if (!agency.isVerified) {
      return res.status(200).json({
        status: false,
        message:
          "You are not verified yet.Wait for verification or contact Bd for further details",
      });
    }
...
exports.getAgencyProfile = async (req, res) => {
  try {
    ...
    let [agency, totalAgencyWiseHost] = await Promise.all([
      Agency.findOne({ _id: req.query.agencyId }),
      User.countDocuments({ isHost: true, hostAgency: req.query.agencyId }),
    ]);
...
exports.agencyWiseHostHistory = async (req, res) => {
  try {
    ...
    const [agency, agencyWiseHost] = await Promise.all([
      Agency.findById(agencyId),
      User.find({ isHost: true, hostAgency: agencyId }),
    ]);
    if (!agency) {
...
exports.createHost = async (req, res) => {
  try {
    ...
    const [agency, user] = await Promise.all([
      Agency.findById(req.query.agencyId),
      User.findOne({ uniqueId: req.query.userId }),
    ]);
    ...
    if (user.isAgency) {
      return res
        .status(200)
        .json({ status: false, message: "User is already an agency." });
    }

    if (agency.isActive === false) {
      return res
        .status(200)
        .json({ status: false, message: "Agency is not active." });
    }

    user.isHost = true;
    user.hostAgency = agency._id;
    user.hostLoginString = config.HOST_PATH + "hostlogin?id=" + user._id,
