...
const Svip = require("../svip/svip.model");
...
exports.index = async (req, res) => {
  try {
    ...
    let blockMatchQuery = {};
    if (req.query.isBlock === "true") {
      blockMatchQuery = { isBlock: true };
    } else if (req.query.isBlock === "false") {
      blockMatchQuery = { isBlock: false };
    }
    // let query;
...
exports.loginSignup = async (req, res) => {
  try {
    ...
    // Step 2: Default Bangladesh
    let userCountry = "BD";
    let countryFlagImage = "https://flagcdn.com/w320/bd.png";

    // Step 3: If IP exists, get country from geoip
    if (ip) {
      const geo = geoip.lookup(ip);
      if (geo && geo.country) {
        userCountry = geo.country;
        countryFlagImage = `https://flagcdn.com/w320/${geo.country.toLowerCase()}.png`;
      }
    }

    console.log("IP:", ip, "Country:", userCountry);
    ...
    const identityData = await User.findOne({
    ...
    if (identityData) {
      return res
        .status(200)
        .json({ status: false, message: "You device blocked by admin!" });
    }
    ...
    if (req.body.loginType == 0) {
      ...
      if (userQuery) {
        ...
        userQuery.country = userCountry;

        userQuery.countryFlagImage = `https://flagcdn.com/w320/${userCountry.toLowerCase()}.png`;
...
exports.signup = async (req, res) => {
  try {
    ...
    const { email, password, gender, country, age } = req.body;
    ...
    newUser.image =
      req.body?.image === ""
        ? req.body?.image.toLowerCase() === "female"
          ? `${config.baseURL}storage/female.png`
          : `${config.baseURL}storage/male.png`
        : req.body.image;
...
exports.getProfile = async (req, res) => {
  try {
    ...
    console.log("svip", user.activeSvip?.image);
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    console.log("updateProfile old user.image:", user.image);

    if (req.files && req.files.image) {
      console.log(
        "updateProfile uploaded image file:",
        req.files.image[0],
      );
      ...
      if (image_[1] !== "/male.png" && image_[1] !== "/female.png") {
        if (fs.existsSync("storage" + image_[1])) {
          console.log("updateProfile deleting old image:", "storage" + image_[1]);
          fs.unlinkSync("storage" + image_[1]);
        } else {
          console.log(
            "updateProfile old image not found on disk:",
            "storage" + image_[1],
          );
        }
      }
      ...
      console.log("updateProfile new user.image:", user.image);
    } else {
      console.log("updateProfile no image file received in req.files.image");
    }
    ...
    await user.save();
    console.log("updateProfile saved user.image:", user.image);
  ...
  try {
    ...
    const [user, profileUser] = await Promise.all([
      ...
      User.findOne({ ...query })
        ...
        .populate({
          path: "uploadTags",
          select: "_id name frame type",
        })
        .select(
          "name username gender age image country uniqueId bio followers following video post level isVIP coverImage loginType avatarFrameImage countryFlagImage isAgency isHost isCoinSeller isOnline lastLogin onlineStatusDate",
        ),
    ]);
    ...
    const rankedUser = await getTopUsers(profileUser._id);

    const coinTotal = await Wallet.aggregate([
      {
        $match: { userId: profileUser._id, diamond: { $ne: null } },
      },
      {
        $group: {
          _id: null,
          totalReceivedCoin: {
            $sum: { $cond: [{ $eq: ["$isIncome", true] }, "$diamond", 0] },
          },
          totalSentCoin: {
            $sum: { $cond: [{ $eq: ["$isIncome", false] }, "$diamond", 0] },
          },
        },
      },
    ]);
    ...
    return res.status(200).json({
      ...
      user: {
        ...
        rankedUser,
        totalReceivedCoin:
          coinTotal.length > 0 ? coinTotal[0].totalReceivedCoin : 0,
        totalSentCoin: coinTotal.length > 0 ? coinTotal[0].totalSentCoin : 0,
      },
    });
  } catch (error) {
    console.log(error);
...
exports.deleteAccount = async (req, res) => {
  try {
    ...
    await Promise.all([
      followerModel.deleteMany({ fromUserId: userId }),
      followerModel.deleteMany({ toUserId: userId }),
      postModel.deleteMany({ userId }),
      videoModel.deleteMany({ userId }),
      // Add other deletions if your schema includes e.g. Chats, Orders, Gifts, etc.
    ]);
...
exports.offlineUser = async (userId) => {
  try {
    ...
    if (user) {
      ...
      user.token = null;
      user.channel = null;
      user.onlineStatusDate = new Date().toLocaleString("en-US", {
        timeZone: "Asia/Kolkata",
      });
      await user.save();
...
exports.editUniqueId = async (req, res) => {
  try {
    const user = await User.findById(req.body.userId);
    if (!user) {
      return res
    ...
    if (req.body.uniqueId && parseInt(req.body.uniqueId) === user.uniqueId) {
      return res.status(200).json({ status: true, message: "Success", user });
    }
    ...
    if (isIdExists) {
      return res.status(400).json({
        status: false,
        message: "Unique ID is already in use by another user",
      });
    }

    if (!Number.isNaN(Number(req.body.uniqueId))) {
      user.uniqueId = Number(req.body.uniqueId);
    }
    await user.save();
    return res.status(200).json({ status: true, message: "Success", user });
  } catch (error) {
...
async function getTopUsers(roomId, limit = null) {
  const now = new Date();
  ...
  return result[0] || { today: [], weekly: [], overAll: [] };
}

exports.getUplaodTag = async (req, res) => {
  try {
    console.log("body", req.body);
    const tag = await User.findOne({ uniqueId: req.body.userId })

      .populate({
        path: "uploadTags",
        select: "_id name frame type text",
      });
...
exports.getUplaodBadge = async (req, res) => {
  try {
    console.log("body", req.body);
    const tag = await User.findOne({ uniqueId: req.body.userId })
...
exports.getUniqueCountries = async (req, res) => {
  try {
    ...
    const hasBD = countries.some((c) => c.country === "BD");
    if (!hasBD) {
      countries.push({
        country: "BD",
...
exports.getUsersForCoinSeller = async (req, res) => {
  try {
    if (req.query.search) {
      const users = await User.aggregate([
        { $addFields: { searchUniqueId: { $toString: "$uniqueId" } } },
      ...
      const users = await User.find({ isFake: false }).select("uniqueId image");
...
exports.updateUserSeller = async (req, res) => {
  try {
    const user = req.admin;

    if (user.role !== "super_coin" && user.role !== "sub_coin_seller") {
      return res
        .status(403)
    ...
    if (user.coinAmount < Number(coin)) {
      return res.status(200).json({
        status: false,
    ...
    const data = await User.findById(coinSellerId);
    ...
    data.diamond = data.diamond + Number(coin);
    await data.save();
  ...
  try {
    ...
    if (req.query.search && req.query.search !== "ALL") {
      ...
      if (!isNaN(req.query.search)) {
        query.$or.push({ uniqueId: Number(req.query.search) });
      }
    } else {
      return res.status(200).json({
        status: true,
