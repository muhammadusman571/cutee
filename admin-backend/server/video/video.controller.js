...
exports.getVideo = async (req, res) => {
  try {
    ...
    const [user, blockedUserIds] = await Promise.all([
      User.findById(new mongoose.Types.ObjectId(req.query.userId))
        .select("_id")
        .lean(),
    ...
    if (settingJSON?.isFake) {
      const combinedVideos = [...video, ...fakeVideo].sort(
        () => 0.5 - Math.random(),
      );
...
// get following feed - if userId given, only videos of users that userId follows; else all videos
exports.getFollowingVideos = async (req, res) => {
  try {
    const { userId, start = 0, limit = 20 } = req.query;

    let matchConditions = { isDelete: false, isFake: false };

    if (userId) {
      const [followingIds, blockedUserIds] = await Promise.all([
        Follower.distinct("toUserId", { fromUserId: userId }),
        Block.distinct("toUserId", { userId }),
      ]);

      matchConditions.userId = { $in: followingIds, $nin: blockedUserIds };
    }
...
exports.updateFakeVideo = async (req, res) => {
  try {
    let [video, song] = await Promise.all([
