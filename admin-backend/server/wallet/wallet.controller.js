...
exports.call = async (req, res) => {
  try {
    ...
    const [callerUser, receiverUser] = await Promise.all([
      User.findById(req.body.callerUserId),
    ...
    const [token, outgoing] = await Promise.all([
      RtcTokenBuilder.buildTokenWithUid(
        global.settingJSON.agoraKey,
        global.settingJSON.agoraCertificate,
        req.body.channel,
        uid,
        role,
        privilegeExpiredTs,
...
exports.history = async (req, res) => {
  try {
    ...
    if (req.body.type === "diamond" || req.body.type === "rCoin") {
      ...
      const da = await Wallet.aggregate([
        {
          $match: {
            userId: user._id,
            isIncome: false,
      ...
      for (let i = 0; i < da.length; i++) {
        const a = await User.findById(da[i].receiverId);

        console.log(a.rCoin, da[i].totalDiamond);

        total += da[i].totalDiamond;

        await User.updateOne(
          { _id: da[i].receiverId },
          {
            $inc: {
              rCoin: -da[i].totalDiamond,
            },
          },
        );
      }
      ...
      return res.status(200).json({
        ...
        outgoing:
          history[0].outgoing.length > 0 ? history[0].outgoing[0].total : 0,
      });
...
exports.liveAnalytic = async (req, res) => {
  try {
    ...
    var todayLiveStreaming = moment
      .utc(todayLiveHistoryTotalDuration.asMilliseconds())
      .format("HH:mm:ss");
    ...
    var weekAudio = moment
      .utc(totalAudioDuration.asMilliseconds())
      .format("HH:mm:ss");
    ...
    var todayAudio = moment
      .utc(todayAudioDuration.asMilliseconds())
      .format("HH:mm:ss");
...
exports.sendGiftFakeHost = async (req, res) => {
  try {
    ...
    const senderUser = await User.findById(req.query.senderUserId).populate(
      "level",
    );
    if (!senderUser) {
      return res.send({
...
exports.teenPatti = async (req, res) => {
  try {
    ...
    if (
      req.query.startDate &&
      req.query.startDate !== "ALL" &&
      req.query.endDate !== "ALL"
    ) {
    ...
    return res.status(200).json({
      ...
      totalPage:
        teenPattiHistory[0]?.pageInfo.length > 0
          ? teenPattiHistory[0]?.pageInfo[0]?.count
          : 0,
...
exports.rouletteCasino = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      adminCoin: adminCoin.coin,
      totalPage:
        rouletteCasinoHistory[0]?.pageInfo.length > 0
          ? rouletteCasinoHistory[0]?.pageInfo[0]?.count
          : 0,
...
exports.ferryWheel = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      adminCoin: adminCoin.coin,
      totalPage:
        ferryWheelHistory[0]?.pageInfo.length > 0
          ? ferryWheelHistory[0]?.pageInfo[0]?.count
          : 0,
...
exports.agencyHistory = async (req, res) => {
  try {
    ...
    const totalHostEarning =
      hostEarningResult.length > 0 ? hostEarningResult[0].totalHostEarning : 0;
    const totalAgencyEarning =
      agencyEarningResult.length > 0
        ? agencyEarningResult[0].totalAgencyEarning
        : 0;
...
exports.allAgencyHistory = async (req, res) => {
  try {
    ...
    if (user.role === "owner") {
      // Owner can see all agencies, no extra filter
    } else if (user.role === "super_admin") {
      // Super admin can see agencies where ref is their _id
      const admins = await Admin.find({ ref: user._id }).select("_id");
      const refIds = [user._id];
      if (admins.length) {
        admins.forEach((admin) => {
          refIds.push(admin._id);
        });
      }
      matchQuery.ref = { $in: refIds };
    } else if (user.role === "admin") {
      // Admin can see agencies where ref is either:
      // 1. Their own _id
      // 2. The super admin they belong to (user.superAdminRef assumed)
      const refIds = [user._id];
      // if (user.ref) refIds.push(user.ref);
      matchQuery.ref = { $in: refIds };
    } else {
    ...
    for (const agency of agencies) {
      ...
      const totalHostEarning =
        hostEarningResult.length > 0
          ? hostEarningResult[0].totalHostEarning
          : 0;
      const totalAgencyEarning =
        agencyEarningResult.length > 0
          ? agencyEarningResult[0].totalAgencyEarning
