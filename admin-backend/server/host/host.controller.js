...
exports.getProfile = async (req, res) => {
  try {
    ...
    }).populate("hostAgency", "name image _id uniqueId");
    if (!host) {
      return res
        .status(200)
...
exports.callHistory = async (req, res) => {
  try {
    ...
    const callHistory = await Wallet.aggregate([
      {
        $match: {
          $or: [
            { userId: new mongoose.Types.ObjectId(hostId) },
            { otherUserId: new mongoose.Types.ObjectId(hostId) },
          ],
          type: 3,
        },
      },
    ...
    return res
      .status(200)
      .json({ status: true, message: "Success", data: callHistory });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
exports.hostCallHistoryForAgency = async (req, res) => {
  try {
    ...
    const startDate = moment(req.query.startDate, "YYYY-MM-DD").format(
      "YYYY-MM-DD"
    );
    const endDate = moment(req.query.endDate, "YYYY-MM-DD").format(
      "YYYY-MM-DD"
    );
...
exports.hostLiveHistoryForAgency = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: true, message: "Success", data: liveHistory });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
    ...
    const data = await LiveStreamingHistory.aggregate([
      ...
      },
      {
        $addFields: {
          ...
          durationMillis: {
            $subtract: [
              { $toLong: { $toDate: "$endTime" } },
              { $toLong: { $toDate: "$startTime" } },
            ],
          },
        },
      },
...
exports.index = async (req, res) => {
  try {
    const currentUser = req.admin; // Logged-in admin user
    ...
    let agencyMatch = {}; // this will be used later in the pipeline
    if (currentUser.role === "owner") {
      // Owner can see all agencies
    } else if (currentUser.role === "super_admin") {
      const admins = await Admin.find({ ref: currentUser._id }).select("_id");
      const refIds = [currentUser._id];
      if (admins.length) {
        admins.forEach((admin) => refIds.push(admin._id));
      }
      agencyMatch["agency.ref"] = { $in: refIds };
    } else if (currentUser.role === "admin") {
      agencyMatch["agency.ref"] = { $in: [currentUser._id] };
    } else {
      return res
        .status(403)
