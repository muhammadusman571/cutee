...
// friends list (mutual follow - i follow them and they follow me back) with live status
exports.friendsList = async (req, res) => {
  try {
    ...
    const [user, blockedUserIds] = await Promise.all([
      ...
      Block.distinct("toUserId", { userId }),
    ]);
    ...
    const friends = await Follower.aggregate([
      { $match: { fromUserId: userId } }, // users i follow
      {
        $lookup: {
          from: "followers",
          let: { theirId: "$toUserId" },
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [
                    { $eq: ["$fromUserId", "$$theirId"] },
                    { $eq: ["$toUserId", userId] },
                  ],
                },
              },
      ...
      { $match: { "followsMeBack.0": { $exists: true } } }, // they follow me back = friend
      {
        $lookup: {
          from: "users",
          localField: "toUserId",
      ...
      { $unwind: "$friend" },
      { $match: { "friend._id": { $nin: blockedUserIds } } },
      {
        $lookup: {
          from: "liveusers",
          localField: "toUserId",
          foreignField: "liveUserId",
          as: "liveRoom",
        },
      },
      { $unwind: { path: "$liveRoom", preserveNullAndEmptyArrays: true } },
      {
        $project: {
          _id: 0,
          userId: "$friend._id",
          ...
          image: "$friend.image",
          avatarFrameImage: "$friend.avatarFrameImage",
          country: "$friend.country",
          isVIP: "$friend.isVIP",
          isOnline: "$friend.isOnline",
          isLive: { $cond: [{ $ifNull: ["$liveRoom._id", false] }, true, false] },
          roomId: { $ifNull: ["$liveRoom.liveStreamingId", null] },
        },
      },
      { $skip: start },
      { $limit: limit },
    ...
    return res.status(200).json({ status: true, message: "Success", friends });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ status: false, error: error.message || "Server Error" });
  }
